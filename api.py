...
# ✅ Persistência (Render Disk) — NÃO é BD
...
# -------------------------
# ✅ HEALTH (para o Render não dormir e para teste rápido)
# -------------------------
@app.get("/health")
def health():
    return jsonify({
...
def book():
    ...
    item = {
        ...
        "date": str(data.get("date", "")).strip(),   # YYYY-MM-DD
        "time": t,                                   # HH:MM
...
# -------------------------
# ✅ SNAPSHOT (OPCIONAL) - se o teu bridge pedir isto
# -------------------------
@app.get("/snapshot")
def snapshot():
    secret = request.args.get("secret", "")
...
# -------------------------
# CLIENTE: VER DIA (sem dados privados)
# -------------------------
def _day_items_for_clients(date: str = "", barber: str = ""):
    ...
    for b in BOOKINGS.values():
        ...
        # histórico não bloqueia o cliente
...
# ==========================
# ✅ ADMIN: PATCH booking (editar status/notes/dur/etc)
# ==========================
...
# ==========================
# ✅ ADMIN: CANCELAR (histórico)
# (mantém item, muda status)
# ==========================
...
# ==========================
# ✅ ALIAS para o botão do MAPA (DELETE)
# - em vez de apagar, CANCELA
# ==========================
@app.delete("/admin/bookings/<bid>")
def admin_delete_as_cancel(bid):
    if not is_admin(request):
        return bad("unauthorized", 401)
    ...
    save_bookings()
    push_change("upsert", BOOKINGS[bid])
    return jsonify({"ok": True, "mode": "cancelled-not-deleted"})
